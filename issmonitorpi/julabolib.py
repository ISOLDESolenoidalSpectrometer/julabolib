...
class JULABO():
	...
	def send_command(self, command=''):
		...
		#self.ser.write( ( command + END_OF_FILE_CHAR ).encode('ascii') )
		self.ser.write( bytes( command + END_CHAR , 'ascii') )
		time.sleep(0.1)
		logging.debug('Command sent to the unit: ' + command)
		response = self.ser.readline()
		if 0 == len(response):
			logging.warning('No response from the unit')
		logging.debug( 'Encoding detected by chardet: ' + str(chardet.detect(response) ) )
